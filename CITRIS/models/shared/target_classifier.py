...
class TargetClassifier(nn.Module):
    ...
    def _step_exp_avg(self):
        ...
        for p1, p2 in zip(self.classifiers.parameters(), self.exp_classifiers.parameters()):
            p2.data.mul_(self.momentum_model).add_(p1.data * (1 - self.momentum_model)) # p2*m + p1*(1-m)
    ...
    def _update_dist(self, target):
        ...
        if self.dist_steps < 1e6:  # At this time we should have a pretty good average
            ...
            self.avg_dist.mul_(self.dist_steps/(self.dist_steps + 1)).add_(new_dist * (1./(self.dist_steps + 1))) # avg_dist*N/(N+1) + new_dist*(1/(N+1))
    ...
    def forward(self, z_sample, target, transition_prior, logger=None):
        ...
        target_assignment = F.gumbel_softmax(transition_prior.target_params[None].expand(batch_size, time_steps, -1, -1), 
                                             tau=self.gumbel_temperature, hard=True) # B_T_Z_C+1
        ...
        target_assignment = target_assignment.permute(0, 1, 3, 2) # B_(T-1)_(C+2)_Z -> B_T_Z_(C+2)
        z_sample = z_sample[...,None,:].expand(-1, -1, num_targets, -1) # B_T_Z -> B_T_(C+2)_Z
        exp_target = target[...,None,:].expand(-1, -1, num_targets, -1).flatten(0, 2).float() # B_(T-1)_C -> B*(T-1)*(C+2)_C


        if self.fullview_baseline:
            loss_mask = torch.ones(num_targets,num_classes,dtype=torch.bool, device=target.device)[None].expand(batch_size * time_steps, -1, -1).flatten(0, 1)
        else:
            ...
            loss_mask = loss_mask[None].expand(batch_size * time_steps, -1, -1).flatten(0, 1) # > B*(T-1)*(C+2)_C
        ...
        model_inps = model_inps.flatten(0, 2) # B_(T-1)_(C+2)_Z*3 -> B*(T-1)*(C+2)_Z*3
        ...
        # region Log target classification accuracies
        if logger is not None and (logger.trainer is None or logger.trainer._results is not None):
            with torch.no_grad():
        ...
        if self.fullview_baseline:
            # Scales down loss mask so the sum matches the non-fullview-baseline-version
            # In this way, the relative size of loss_z and loss_model is the same
            loss_mask /= (num_targets / (num_targets - num_classes))
