...
def gaussian_log_prob(mean, log_std, samples, std_minimum=0):
    ...
    return - log_std - 0.5 * np.log(2*np.pi) - 0.5 * ((samples - mean) / (log_std.exp() + std_minimum)) ** 2 # First term becomes more positive (better) as std gets smaller, second term becomes more negative (worse) as std gets smaller. The error (samples - mean) determines the direction of the gradient for the std.
    # return - (log_std.exp()  + std_minimum).log() - 0.5 * np.log(2*np.pi) - 0.5 * ((samples - mean) / (log_std.exp() + std_minimum)) ** 2 # this can lead to overflow if max log std is too large
...
def log_matrix(matrix, trainer, name, current_epoch=None, log_dir=None):
    ...
    if log_dir is None:
        log_dir = trainer.logger.experiment.dir  # Nathan: logger.experiment.dir instead of logger.log_dir for wandb logger
    filename = os.path.join(log_dir, name + '.npz')
    if log_dir == '/tmp':
        filename = os.path.join(log_dir, name + f'_{"_".join([str(i) for i in matrix.shape])}.npz')
