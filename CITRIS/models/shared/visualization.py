...
def visualize_ae_reconstruction(model, images):
    ...
    # Nathan: some changes to get horizontal instead of vertical
    # out_images = torch.stack([images, reconst], dim=1).flatten(0, 1)
    out_images = torch.stack([images, reconst], dim=0).flatten(0, 1)
    # img_grid = torchvision.utils.make_grid(out_images, nrow=2, normalize=True, pad_value=0.5)
    img_grid = torchvision.utils.make_grid(out_images, nrow=8, normalize=True, pad_value=0.5)
...
@torch.no_grad()
def plot_target_assignment(prior, dataset=None, target_assignment=None):
    """ Plots the probability matrix of latent-to-causal variable assignments, or the given target assignment """
    if target_assignment is not None:
        target_probs = target_assignment.detach().cpu().numpy()
    else:
...
@torch.no_grad()
def visualize_frame_prediction(model, img_pair, It1, dataset=None, *args, **kwargs):
    xt, xt1 = img_pair[:,0], img_pair[:,1]
    xt1_pred = model.sample_xt1(xt, It1)
    bs = xt.shape[0]

    fig, axes = plt.subplots(nrows=3, ncols=bs, figsize=(bs*2,3*2))
    if bs == 1:
        axes = [[ax] for ax in axes]

    def prep_img_for_plot(img):
        return (img.detach().cpu().numpy().transpose(1, 2, 0) + 1) / 2

    rows = ['Xt', 'Xt1', 'Xt1_pred']
    imgs = [[prep_img_for_plot(img) for img in row] for row in [xt, xt1, xt1_pred]]
    for i, ax_row in enumerate(axes):
        for j, ax in enumerate(ax_row):
            ax.imshow(imgs[i][j])
            if j == 0:
                ax.set_ylabel(rows[i], rotation=0, size='large', labelpad=30)
            ax.set_xticks([])
            ax.set_yticks([])
    fig.tight_layout()
    return fig
...
@torch.no_grad()
def visualize_triplet_reconstruction(model, img_triplet, labels, sources, dataset=None, use_cheating_target_assignment=False, *args, **kwargs):
    """ Plots the triplet predictions against the ground truth for a VAE/Flow """

    if 'fig_and_axes' in kwargs:
        fig, axes = kwargs['fig_and_axes']
    else:
        fig, axes = plt.subplots(1, 5, figsize=(8, 3))

    sources = sources[0].to(model.device)
    labels = labels[-1]
    triplet_rec = model.triplet_prediction(img_triplet[None], sources[None],use_cheating_target_assignment=use_cheating_target_assignment)
    ...
    fig.suptitle(f'Image 1: {[targets[i] for i in s1]}, \r\nImage 2: {[targets[i] for i in s2]}')
    plt.tight_layout()
    return fig

def visualize_triplet_reconstruction_parallel(model, img_triplets, labels, sources_list, dataset=None, use_cheating_target_assignment=False,*args, **kwargs):
    """ Plots the triplet predictions against the ground truth for a VAE/Flow, for multiple triplets in parallel """
    fig = plt.figure(figsize=(8, 3*len(img_triplets)))
    subfigs = fig.subfigures(len(img_triplets), 1) if len(img_triplets) > 1 else [fig.subfigures(len(img_triplets), 1)]
    for i, (img_triplet, label, *source_list, subfig) in enumerate(zip(img_triplets, labels, *sources_list, subfigs)):
        visualize_triplet_reconstruction(model, img_triplet, label, source_list, dataset=dataset, fig_and_axes=(subfig, subfig.subplots(1, 5)), use_cheating_target_assignment=use_cheating_target_assignment)
    return fig
