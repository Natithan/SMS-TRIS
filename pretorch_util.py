...
def assign_visible_gpus(free_mem_based=False):
    ...
    #     print("=" * 10, f"Manually setting os.environ['CUDA_VISIBLE_DEVICES'] to {sys.argv[sys.argv.index('--visible_gpus') + 1]}",
    #           "=" * 10)
    #     os.environ['CUDA_VISIBLE_DEVICES'] = sys.argv[sys.argv.index('--visible_gpus') + 1]
    #     del sys.argv[sys.argv.index('--visible_gpus'):sys.argv.index('--visible_gpus') + 2]
    # else:
    #     if 'CUDA_VISIBLE_DEVICES' in os.environ:
    #         print(os.environ['CUDA_VISIBLE_DEVICES'])
    #     else:
    #         print("os.environ['CUDA_VISIBLE_DEVICES'] not set")
    #         os.environ['CUDA_VISIBLE_DEVICES'] = ",".join([str(i) for i in get_free_gpus(only_one=only_one)])
    #         print("Now set to", os.environ['CUDA_VISIBLE_DEVICES'])

    # allow either --visible_gpus or --gpus
    arg = '--visible_gpus' if '--visible_gpus' in sys.argv else '--gpus'
    if arg in sys.argv and sys.argv[sys.argv.index(arg) + 1]:
        print("=" * 10, f"Manually setting os.environ['CUDA_VISIBLE_DEVICES'] to {sys.argv[sys.argv.index(arg) + 1]}",
              "=" * 10)
        os.environ['CUDA_VISIBLE_DEVICES'] = sys.argv[sys.argv.index(arg) + 1]
